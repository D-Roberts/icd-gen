...
def loss_if_predict_linalg_shrunken(
    criterion,
    dataloader,
    data_label,
    sig2_z,
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader, 0):
            ...
            for b in range(nbatch):
                ...
                x_L_corrupt = np.expand_dims(
                    x_L_corrupt, axis=1
                )  # this is needed for linalg shape inference in fn
                project_xL_onto_W_np = proj_affine_subspace_estimator(
                    X_seq, x_L_corrupt
                )
    ...
    if print_val:
        print(
            "\t%s data loss if linalg. projection (shrunk): %.3e (count=%d)"
            % (data_label, mse_loss, count)
        )
    return mse_loss
...
def loss_if_predict_linalg(criterion, dataloader, data_label, print_val=True):
    ...
    if print_val:
        print(
            "\t%s data loss if linalg. projection: %.3e (count=%d)"
            % (data_label, mse_loss, count)
        )
    return mse_loss
...
def loss_if_predict_subsphere_baseline(
    criterion,
    dataloader,
    data_label,
    sphere_radius,
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader, 0):
            ...
            for b in range(nbatch):
                ...
                project_xL_onto_W_np, est_basis = proj_affine_subspace_estimator(
                    X_seq, x_L_corrupt, return_basis=True
                )
                project_xL_onto_W_np = np.squeeze(project_xL_onto_W_np)
                ...
                # ===================================================================================
                radius_est = np.mean(
                    np.linalg.norm(X_seq[:, 0], axis=0)
                )  # for now, we assert style_origin_subspace

                # 3) estimate sphere dimension from the sequence
                # ===================================================================================
                d_dim_infer = est_basis.shape[
                    1
                ]  # for a circle in 2d, this would be "2"; for the bessel ratio fn we then pass d-1
                ...
                shrink_factor = bessel_ratio_subhalf_sub3half(
                    d_dim_infer - 1, beta_val
                )  # note we pass d-1; a circle is 2d and we pass 1
                prediction_on_subsphere = (
                    radius_est * project_xL_onto_W_np / x_L_corrupt_projected_norm
                )

                if shrunken:
                    if sphere2_force_wolfram:
                        shrink_factor_wolfram = (
                            1 / beta_val * (beta_val / np.tanh(beta_val) - 1)
                        )
                        assert (
                            d_dim_infer == 3.0
                        )  # i.e. points lie in 3d, on a 2-sphere
    ...
    if print_val:
        print(
            "\t%s data loss if projection onto subsphere: %.3e (count=%d)"
            % (data_label, mse_loss, count)
        )
    return mse_loss
