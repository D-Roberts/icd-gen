...
class TransformerModelV10(nn.Module):
    ...
    def __init__(self, context_length, dim_input, dim_attn=None, n_layer=1, n_head=2):
        ...
        self.linear = nn.Linear(context_length-1, context_length-1, bias=None) #the context_len includes the token to be corrupted
    ...
    def forward(self, xs):
        ...
        #first part 
        k_heads = xs_skip_last.view(xs_skip_last.shape[0], xs_skip_last.shape[1], self.num_heads, self.head_dim).transpose(1, 2)
        kwkq = k_heads @ W_KQ
        q_last = xs[:, :, [-1]]
        q = q_last.view(q_last.shape[0], q_last.shape[1], self.num_heads, self.head_dim).transpose(1, 2)
        attn_arg = kwkq @ q / self.rho

        # p7 Bartlett: "Softmax applied column-wise" (dim = data dim, not token dim)
        softmax_attn_arg = torch.softmax(attn_arg, dim=1)
        f_attn = W_PV @ k_heads @ softmax_attn_arg  # the residual stream term "+ xs" has been removed
...
class TransformerModelV10noresOmitLast(TransformerModelV2):
    ...
    def forward(self, xs):
        ...
        xs_skip_last = xs[:, :, :-1]
        attn_arg = torch.transpose(xs_skip_last, 1, 2) @ torch.concatenate((W_KQ, W_KQ1), axis=0) / self.rho #this thing where only last token so dim is 1 and not cont_elnt - 1
        attn_arg1 = torch.transpose(xs_skip_last, 1, 2) @ W_KQ1 @ xs[:, :, [-1]] / self.rho
...
class TransformerModelV4noresOmitLast(nn.Module):
    ...
    def forward(self, xs):
        ...
        #attn_arg = torch.transpose(xs_skip_last, 1, 2) @ W_KQ @ xs_skip_last / rho
        projection_estimate = xs_skip_last @ torch.transpose(xs_skip_last, 1, 2) / self.rho
        ...
        out = f_attn_approx1[:, :, -1]  # take dim_n output result at last token, for all batches
...
MODEL_CLASS_FROM_STR = {
    ...
    'TransformerModelV3noresOmitLast':
        {'class': TransformerModelV3noresOmitLast, 'alias': 'TV3nrOL'},
    'TransformerModelQKVnores':
        {'class': TransformerModelQKVnores, 'alias': 'TQKVnr'},
    'TransformerModelV4noresOmitLast':
        {'class': TransformerModelV4noresOmitLast, 'alias': 'TV4nrOL'},
    'TransformerModelV6noresOmitLast':
        {'class': TransformerModelV6noresOmitLast, 'alias': 'TV6nrOL'},
    'TransformerModelV7noresOmitLast':
        {'class': TransformerModelV7noresOmitLast, 'alias': 'TV7nrOL'},
    'TransformerModelV8noresOmitLast':
        {'class': TransformerModelV8noresOmitLast, 'alias': 'TV8nrOL'},
