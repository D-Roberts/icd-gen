...
from data_util import reload_lossbaseline_dict
...
def vis_weights_kq_pv(
    ...
    plt.plot(
        W_KQ_rowsum_minus_diag,
        "--o",
        label="KQ sum(abs(row)) - diag",
        color=line1.get_color(),
        markerfacecolor="None",
    )
    (line2,) = plt.plot(np.diag(learned_W_PV), "-o", label="PV diag")
    plt.plot(
        W_PV_rowsum_minus_diag,
        "--o",
        label="PV sum(abs(row)) - diag",
...
def vis_weights_kq_pv_multirow(
    learned_W_KQ_list,
    learned_W_PV_list,
    titlemod="",
    dir_out=None,
    ...
    if global_clims:
        ...
        all_data = np.concatenate(
            [np.ravel(mat) for mat in (learned_W_KQ_list + learned_W_PV_list)]
        )
    ...
    for k in range(nrows):
        ...
        if global_clims:
            ...
            combined_data = np.concatenate(
                [learned_W_KQ_list[k].ravel(), learned_W_PV_list[k].ravel()]
            )
        ...
        ax0.set_title(
            r"$W_{KQ}, \langle w_{ii} \rangle = %.3f$"
            % np.mean(np.diag(learned_W_KQ_list[k]))
        )

        im1 = ax1.imshow(learned_W_PV_list[k], cmap=cmap, norm=row_norm)
        ax1.set_title(
            r"$W_{PV}, \langle w_{ii} \rangle = %.3f$"
            % np.mean(np.diag(learned_W_PV_list[k]))
        )
...
def vis_loss(dir_curves, titlemod="", dir_out=None, fname=None, flag_show=False):
    ...
    learned_W_PV = net.W_PV.detach().numpy()
    """
    curve_train_eavg = np.load(
        dir_curves + os.sep + "curve_losstrain_epoch_avg.npz", allow_pickle=True
    )
    curve_train_interval = np.load(
        dir_curves + os.sep + "curve_losstrain_interval.npz", allow_pickle=True
    )
    curve_train_tbatch = np.load(
        dir_curves + os.sep + "curve_losstrain_tbatch.npz", allow_pickle=True
    )
    curve_test_interval = np.load(
        dir_curves + os.sep + "curve_losstest_interval.npz", allow_pickle=True
    )
    ...
    if "dumb_A_mse_on_train" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["dumb_A_mse_on_train"], label="train (guess 0)", color="grey"
        )
    if "dumb_A_mse_on_test" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["dumb_A_mse_on_test"],
            linestyle="--",
            label="test (guess 0)",
            color="grey",
        )
    ...
    if "heuristic_mse_on_train" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["heuristic_mse_on_train"],
            label=r"train ($P_W$)",
            color="black",
        )
    if "heuristic_mse_on_test" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["heuristic_mse_on_test"],
            linestyle="--",
            label=r"test ($P_W$)",
            color="black",
        )
    if "heuristic_mse_shrunken_on_train" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["heuristic_mse_shrunken_on_train"],
            label=r"train ($P_W$ shrunk)",
            color="purple",
        )
    if "heuristic_mse_shrunken_on_test" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["heuristic_mse_shrunken_on_test"],
            linestyle="--",
            label=r"test ($P_W$ shrunk)",
            color="purple",
        )
    if "theory_expected_error_linalg_shrunken" in loss_baselines.keys():
        plt.axhline(
            loss_baselines["theory_expected_error_linalg_shrunken"],
            linestyle=":",
            color="purple",
            label=r"$L(\theta^*)$",
        )
